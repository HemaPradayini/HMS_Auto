 ...
 *
 */
public class OPToIPWorkflowWithInsuranceCopyChargesBillAction {
...
//	public void BeforeTest(){
	private void login(){
		AutomationID = "Login - OPToIPWorkflowWithInsuranceCopyChargesBillAction";
		DataSet = "TS_037";
	...
	public void OPToIPWorkflowWithInsuranceCopyChargesBillAction(){
		openBrowser();
		login();
		DataSet = "TS_037A";
		...
		SearchBillList searchBills = new SearchBillList(executeStep, verifications);
...
//		navigation.navigateTo(driver, "SearchOpenBills", "BillListPage");
	...
	//	//mrnoSearch.searchMRNo("BillListMRNoField", "BillListMRNoList", "BillListSearchButton", "BillsListResultsTable");	
		//PatientBill patientBill = new PatientBill(executeStep, verifications);		
		patientBill.sponsorSettlementDetails();
		patientBill.setBillStatus("");
...
//		patientBill.settleBillDetails("YES", "SettlementPaymentType");
		...
		navigation.navigateTo(driver, "BillsLink", "BillListPage");
		mrnoSearch.searchMRNo("BillListMRNoField", "BillListMRNoList", "BillListSearchButton", "BillListPatientBills");
		executeStep.performAction(SeleniumActions.Click, "","BillListSecondBillLink");//changed this because after closing the second bill and navigating back to the searchbills, the third bill is actually the second bill.
		verifications.verify(SeleniumVerifications.Appears, "","BillListViewEditMenu",false);
		executeStep.performAction(SeleniumActions.Click, "","BillListViewEditMenu");
		verifications.verify(SeleniumVerifications.Appears, "","PatientBillScreen",false);
		executeStep.performAction(SeleniumActions.Click, "", "CloseTour");
...
//		}
		...
		System.out.println("TS_037 completed Successfully");
	...
	public void performOrderedTestsAndSignOffReports(SiteNavigation navigation){
		...
		for (int i=1; i<= rowCount; i++){
			...
			System.out.println("no of pending bills is :: " + rowCount);
			...
			search.searchMRNo("LabPendingTestPatientNameField","LabPendingTestsPatientOPNoSelectDiv","LabPendingTestSearchButton","LabPendingTests");
			rowCount = this.executeStep.getDriver().findElements(By.xpath("//table[@id='resultTable']/tbody/tr")).size();
		}
